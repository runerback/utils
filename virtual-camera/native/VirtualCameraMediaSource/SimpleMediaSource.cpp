...
namespace winrt::WindowsSample::implementation
{
    ...
    HRESULT SimpleMediaSource::Initialize(_In_ IMFAttributes* pAttributes)
    {
        ...
        wil::unique_cotaskmem_string imagePath;
        UINT32 imagePathLength = 0;
        std::wstring configuredImagePath;
        ...
        for (unsigned int i = 0; i < NUM_STREAMS; i++)
        {
            ...
            if (SUCCEEDED(m_spAttributes->GetAllocatedString(VCAM_IMAGE_PATH, &imagePath, &imagePathLength)))
            {
                configuredImagePath.assign(imagePath.get(), imagePathLength);
            }
            ...
            RETURN_IF_FAILED(m_streamList[i]->Initialize(
                ...
                configuredImagePath.empty() ? nullptr : configuredImagePath.c_str()));
